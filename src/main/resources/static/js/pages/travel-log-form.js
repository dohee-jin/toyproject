...
const TravelLogFormPage = () => {
    ...
    const onSubmit = async (e) => {
        ...
        // 2. 여행 일지 insert 를 위한 payload 작성
        const payload = {
    ...
    const init = async () => {
        ...
        $.$tagInput?.addEventListener('keydown', async (e) => {
            ...
            if (existing) {
                addTagChip(existing);
            } else { // 검색결과가 없으면 id를 null 로 해서 자바스크립트 배열에 추가
                addTagChip({ id: null, name, category, color: '#6c757d' });
            }

            console.log(`현재 사용자가 선택한 hashtag:`, state.selectedTags);
            $.$tagInput.value = '';
            $.$tagSuggestions.style.display = 'none';
        });
    ...
    const ensureNewTagsCreated = async () => {
        ...
        // 사용자가 선택한 새시태그 배열에서 id가 없는 태그들만 필터링
        // id가 없는 건 db에 없는 새로운 해시태그들이다.
