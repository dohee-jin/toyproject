...
const SignupPage = () => {
    ...
    const checkDuplicateUsername = async (username) => {
        ...
        try {
            ...
            // 중복
            if(response.data) {
                updateInputState(state.$usernameInput, false, response.message
                );
            } else {
                updateInputState(state.$usernameInput, true, response.message);
            }
        } catch (error) {
    ...
    const checkDuplicateEmail = async (email) => {
        ...
        try {
            ...
            // 중복
            if(response.data) {
                updateInputState(state.$emailInput, false, response.message
                );
            } else {
                updateInputState(state.$emailInput, true, response.message);
            }
        } catch (error) {
            console.log(error.message)
        }
    ...
    // 사용자명 입력 이벤트 처리
    const handlerUsernameInput =debounce(e => {
        ...
        if(username.length < 3 || username.length > 15) {
            updateInputState(state.$usernameInput, false, `사용자명은 3~15자 사이여야 합니다.`);
            return;
        }
